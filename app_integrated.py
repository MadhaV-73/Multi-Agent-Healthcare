...
def load_medicine_reference() -> tuple[list[str], list[str], list[str]]:
    ...
    # Only include chest X-ray / respiratory-related symptoms
    CHEST_RESPIRATORY_SYMPTOMS = {
        'cough', 'fever', 'cold', 'bronchitis', 'congestion', 'chest', 
        'breathing', 'shortness', 'breath', 'wheezing', 'pneumonia',
        'respiratory', 'lung', 'mucus', 'phlegm', 'dry', 'wet',
        'pain', 'headache', 'fatigue', 'weakness', 'inflammation'
    }
    ...
    for entry in meds_df["indication"].dropna():
        for token in str(entry).split():
            ...
            # Only add if it's a chest/respiratory related symptom
            if clean and clean in CHEST_RESPIRATORY_SYMPTOMS:
...
def home_page():
    ...
    with col2:
        st.markdown("""
        **✨ Key Features:**
        - 🫁 Chest X-ray analysis for respiratory conditions
        - 💊 Drug interaction checking
        - 🏥 Pharmacy inventory matching
        - 📍 Distance-based recommendations
        - 👨‍⚕️ Pulmonologist booking for escalations
    ...
    st.markdown("""
    1. **Upload Chest X-Ray** - Go to 🩻 X-Ray Analysis page
    2. **Enter Respiratory Symptoms** - Describe cough, fever, breathing issues
    3. **Get AI Analysis** - Receive condition detection (pneumonia, bronchitis, etc.)
    4. **View OTC Recommendations** - See respiratory medicines and nearby pharmacies
    5. **Book Pulmonologist** - If prescription or escalation needed
    """)


def patient_intake_page():
    """Simplified patient intake for chest X-ray and respiratory analysis."""
    st.markdown("""
    <div style="background: linear-gradient(90deg, #43cea2 0%, #185a9d 100%); padding: 1.5rem; border-radius: 10px; margin-bottom: 1rem;">
        <h2 style="color: white; margin: 0;">📝 Patient Intake - Chest X-Ray Analysis</h2>
        <p style="color: #f0f0f0; margin: 0.5rem 0 0 0;">Register patient for respiratory condition assessment and chest X-ray analysis.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with loc_col1:
        selected_city = st.selectbox("City", city_options, key="patient_city_intake", index=0)
    with loc_col2:
        # Filter pincodes by selected city - FIXED: uses correct column name
        ...
        if not pincode_options:
            ...
            pincode = st.text_input("Enter Pincode", help="6-digit pincode", key="patient_pincode_manual_intake")
        else:
            pincode = st.selectbox(
                "Pincode",
                pincode_options,
                help=f"Available pincodes for {selected_city}",
                key="patient_pincode_intake",
                index=0
            )
...
def xray_analysis_page():
    """Chest X-Ray analysis for respiratory conditions - Full multi-agent pipeline"""
    st.markdown("""
    <div style="background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); padding: 1.5rem; border-radius: 10px; margin-bottom: 1rem;">
        <h2 style="color: white; margin: 0;">� Chest X-Ray Analysis & Respiratory Treatment</h2>
        <p style="color: #f0f0f0; margin: 0.5rem 0 0 0;">AI-powered detection: Pneumonia, Bronchitis, TB, COVID-19 suspect, Normal</p>
    </div>
    """, unsafe_allow_html=True)

    st.info("⚠️ Educational demo only — detects respiratory conditions from chest X-rays. NOT medical advice. Call emergency (911/108) if in distress.")
    st.caption("Pipeline: 1) Ingestion → 2) Chest Imaging AI → 3) OTC Respiratory Therapy → 4) Pharmacy Match → 5) Pulmonologist Booking")
    ...
    with demo_col1:
        ...
        if demo_path.exists():
            ...
            if st.button("📥 Load Demo X-Ray", use_container_width=True, key="load_demo_btn"):
                # Store demo X-ray bytes in session state
                with open(demo_path, 'rb') as f:
                    st.session_state.demo_xray_bytes = f.read()
                    st.session_state.demo_xray_loaded = True
                st.success("✅ Demo X-ray loaded! Scroll down and click 'Run complete agent pipeline'")
                st.rerun()
        else:
    ...
    with demo_col2:
        ...
        **Quick Demo Test Case:**
        - Click "Load Demo X-Ray" to automatically use this sample chest X-ray
        - Pre-filled with sample patient data
        - Perfect for testing the AI analysis pipeline
        - Shows how the system detects respiratory conditions
        """)
        
        # Show if demo is loaded
        if st.session_state.get('demo_xray_loaded', False):
            st.success("✅ **Demo X-ray is loaded and ready!**")
            if st.button("🗑️ Clear Demo", key="clear_demo_btn"):
                st.session_state.demo_xray_loaded = False
                st.session_state.demo_xray_bytes = None
                st.rerun()
    
    st.markdown("---")
    
    # Location selection FIRST - OUTSIDE form for dynamic updates
    ...
    with loc_col1:
        city_options = zip_df["city"].unique().tolist()
        selected_city = st.selectbox("City", city_options, help="Select your city", key="xray_city_select", index=0)
    with loc_col2:
        # Filter pincodes by selected city - THIS WORKS BECAUSE IT'S OUTSIDE THE FORM
        city_subset = zip_df[zip_df["city"] == selected_city]
        pincode_options = city_subset["pincode"].tolist() if not city_subset.empty else []
        
        if not pincode_options:
            st.warning(f"No pincodes found for {selected_city}")
            zip_code = st.text_input("Enter ZIP/PIN Code", help="6-digit pincode", key="xray_pincode_manual_input")
        else:
            zip_code = st.selectbox(
                "ZIP / PIN Code",
                pincode_options,
                help=f"Available pincodes for {selected_city} - helps pharmacy matcher",
                key="xray_pincode_select",
                index=0
            )
    ...
    with st.form("xray_form"):
        ...
        # Check if demo X-ray is loaded and show it
        demo_xray_loaded = st.session_state.get('demo_xray_loaded', False)
        if demo_xray_loaded:
            st.success("✅ Demo X-ray loaded! This will be used for analysis.")
            demo_bytes = st.session_state.get('demo_xray_bytes')
            if demo_bytes:
                st.image(demo_bytes, caption="Demo X-Ray Loaded", width=200)
        
        xray_file = st.file_uploader(
            "Chest X-ray (PNG/JPG)" + (" - Demo already loaded, or upload your own" if demo_xray_loaded else ""),
            type=['png', 'jpg', 'jpeg'],
    ...
    # Handle submission - use demo if loaded, otherwise use uploaded file
    if submitted:
        xray_to_use = None
        xray_filename = None
        
        # Priority: uploaded file > demo X-ray
        if xray_file is not None:
            xray_to_use = xray_file.getvalue()
            xray_filename = xray_file.name
            xray_content_type = xray_file.type
        elif demo_xray_loaded and st.session_state.get('demo_xray_bytes'):
            xray_to_use = st.session_state.demo_xray_bytes
            xray_filename = "demo_xray.png"
            xray_content_type = "image/png"
        ...
        with col1:
            st.image(xray_to_use, caption="X-Ray for Analysis", use_container_width=True)
        ...
        with col2:
            ...
            # Prepare data
            multipart_files = [
                ('file', (xray_filename, xray_to_use, xray_content_type))
            ]
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Clear demo X-ray after successful analysis
                    if st.session_state.get('demo_xray_loaded', False):
                        st.session_state.demo_xray_loaded = False
                        st.session_state.demo_xray_bytes = None
...
def display_analysis_results(result):
    ...
    if status == "EMERGENCY":
        ...
        if pharmacy:
            ...
            # Display location info prominently with match quality
